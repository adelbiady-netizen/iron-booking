...
router.get('/pos/admin/diagnose', async (req, res) => {
    ...
    `;
        ...
        const ibTables = await prisma_1.prisma.table.findMany({
    ...
    try {
        const totalTables = await prisma_1.prisma.table.count({ where: { restaurantId } });
        const withAtlasId = await prisma_1.prisma.table.count({ where: { restaurantId, atlasTableId: { not: null } } });
        const sampleTable = await prisma_1.prisma.table.findFirst({
            where: { restaurantId },
            select: { id: true, name: true, atlasTableId: true },
            orderBy: { name: 'asc' },
        });
        // Pick any reservation that has a tableId set
        const sampleResWithTable = await prisma_1.prisma.reservation.findFirst({
        ...
        if (sampleResWithTable?.tableId) {
            const t = await prisma_1.prisma.table.findUnique({
...
router.post('/pos/admin/populate-atlas-table-ids', async (req, res) => {
    const adminSecret = process.env.POS_ADMIN_SECRET;
    if (!adminSecret || req.headers['x-admin-secret'] !== adminSecret) {
        res.status(401).json({ error: 'UNAUTHORIZED' });
        return;
    ...
    const dirRes = await fetch(`${config.posApiBase}/api/v1/hospitality/table-directory?location_id=${config.atlasLocationId}`, { headers: { Authorization: `Bearer ${config.hospitalitySecret}` } });
    if (!dirRes.ok) {
        res.status(502).json({ error: 'ATLAS_ERROR', status: dirRes.status, body: await dirRes.text() });
        return;
    }
    const dir = await dirRes.json();
    const atlasTables = ('data' in dir ? dir.data.tables : dir.tables) ?? [];
    const ironTables = await prisma_1.prisma.table.findMany({ where: { restaurantId } });
    const matched = [];
    const skipped = [];
    for (const at of atlasTables) {
        if (!at.active) {
            skipped.push({ atlasName: at.name ?? at.number, atlasId: at.table_id, reason: 'inactive' });
            continue;
        }
        const label = at.name ?? at.number;
        const iron = ironTables.find(t => t.name === label ||
            ...
            t.name.replace(/\s+/g, '') === label.replace(/\s+/g, ''));
        if (!iron) {
            skipped.push({ atlasName: label, atlasId: at.table_id, reason: 'no_ib_name_match' });
            continue;
        }
        await prisma_1.prisma.table.update({ where: { id: iron.id }, data: { atlasTableId: at.table_id } });
