...
exports.suggestTables = suggestTables;
 ...
 */
async function suggestTables(ctx) {
    const [tables, combinations, restaurantSettings] = await Promise.all([
        ...
        prisma_1.prisma.tableCombination.findMany({
            where: { restaurantId: ctx.restaurantId, isActive: true },
            include: {
        ...
        prisma_1.prisma.restaurant.findUniqueOrThrow({
            where: { id: ctx.restaurantId },
            select: { settings: true },
        }),
    ]);
    const availability = await (0, availability_1.getTableAvailability)(ctx.restaurantId, ctx.date, ctx.time, ctx.durationMinutes, ctx.bufferMinutes);
    const availableIds = new Set(availability.filter((a) => a.isAvailable).map((a) => a.tableId));
    const suggestions = [];
    // Score single tables
    for (const table of tables) {
        if (table.minCovers > ctx.partySize || table.maxCovers < ctx.partySize)
            continue;
    ...
    for (const combo of combinations) {
        if (combo.minCovers > ctx.partySize || combo.maxCovers < ctx.partySize)
            continue;
        if (!availableIds.has(combo.tableAId) || !availableIds.has(combo.tableBId))
            continue;
...
function scoreCombo(combo, ctx) {
    ...
    const reasons = ['Combined table setup'];
    const warnings = ['Requires staff to combine tables before seating'];
...
//# sourceMappingURL=tableMatcher.js.map