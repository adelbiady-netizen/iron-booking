...
router.get('/summary', validate(DateRangeSchema, 'query'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [totalReservations, totalCovers, byStatus] = await Promise.all([
      prisma.reservation.count({ where: { restaurantId, date: { gte: from, lte: to } } }),
...
router.get('/covers-by-hour', validate(DateRangeSchema, 'query'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { dateFrom, dateTo } = req.query as any;
    const restaurantId = req.auth.restaurantId;
...
router.get('/turn-times', validate(DateRangeSchema, 'query'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { dateFrom, dateTo } = req.query as any;
    const restaurantId = req.auth.restaurantId;
    ...
    const turns = completed.map((r) => ({
      actualMinutes: differenceInMinutes(r.completedAt!, r.seatedAt!),
    ...
    const byPartySize: Record<number, { count: number; totalMinutes: number }> = {};
    for (const t of turns) {
      const key = Math.min(t.partySize, 8); // group 8+
      if (!byPartySize[key]) byPartySize[key] = { count: 0, totalMinutes: 0 };
