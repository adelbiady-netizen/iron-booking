...
router.post('/pos/admin/copy-hospitality-secret', async (req: Request, res: Response) => {
  const adminSecret = process.env.POS_ADMIN_SECRET;
  if (!adminSecret || req.headers['x-admin-secret'] !== adminSecret) {
    res.status(401).json({ error: 'UNAUTHORIZED' });
    return;
  ...
  const donor = await prisma.posConfig.findUnique({ where: { restaurantId: fromRestaurantId } });
  if (!donor) {
    res.status(404).json({ error: 'DONOR_NOT_FOUND' });
    return;
  }

  const target = await prisma.posConfig.findUnique({ where: { restaurantId: toRestaurantId } });
  if (!target) {
...
router.get('/pos/admin/diagnose', async (req: Request, res: Response) => {
  const adminSecret = process.env.POS_ADMIN_SECRET;
  if (!adminSecret || req.headers['x-admin-secret'] !== adminSecret) {
    res.status(401).json({ error: 'UNAUTHORIZED' });
    return;
  ...
  try {
    const rows = await prisma.$queryRaw<Array<{ event_id: string; received_at: Date; payload_text: string }>>`
  ...
  try {
    const total         = await prisma.table.count({ where: { restaurantId } });
