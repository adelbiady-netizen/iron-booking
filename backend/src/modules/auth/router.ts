  ...
  router.post('/dev-login', async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      try {
        ...
        const msg: string = dbErr?.message ?? String(dbErr);
      ...
      // ── 5. Upsert tables ─────────────────────────────────────────────────
      const tableDefs = [
        { name: 'T1', sectionId: mainDining.id, minCovers: 2, maxCovers: 4, shape: 'RECTANGLE' as const },
        { name: 'T2', sectionId: mainDining.id, minCovers: 2, maxCovers: 4, shape: 'RECTANGLE' as const },
        { name: 'T3', sectionId: mainDining.id, minCovers: 4, maxCovers: 6, shape: 'ROUND'     as const },
        { name: 'T4', sectionId: mainDining.id, minCovers: 4, maxCovers: 8, shape: 'RECTANGLE' as const },
        { name: 'B1', sectionId: bar.id,        minCovers: 1, maxCovers: 2, shape: 'SQUARE'    as const },
      ];
  ...
  router.post('/dev-super-login', async (_req: Request, res: Response) => {
    ...
    try {
      ...
      const restaurant = await prisma.restaurant.upsert({
        ...
        update: {},
        create: {
          name: 'System', slug: SYSTEM_SLUG, isSystem: true,
      ...
      const passwordHash = await bcrypt.hash('dev123', 10);
      const user = await prisma.user.upsert({
        where:  { restaurantId_email: { restaurantId: restaurant.id, email: DEV_SUPER_EMAIL } },
        update: {},
        create: {
          restaurantId: restaurant.id,
      ...
      res.json({
        token,
        note: 'DEV SUPER ADMIN TOKEN — not available in production',
        user: {
          id: user.id, email: user.email,
          firstName: user.firstName, lastName: user.lastName,
          role: user.role, restaurant: null,
        },
      });
    } catch (err: any) {
